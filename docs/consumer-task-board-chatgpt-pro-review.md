# 面向個人使用者的三欄任務看板：ChatGPT Pro 協作與驗收記錄
...
- 主看板固定顯示 `todo`、`in_progress`、`in_review` 三欄。
- 三欄文案改為“待處理”“處理中”“等你確認”。
- `backlog`、`blocked`、`done`、`canceled` 收入右側“其他任務”面板。
- 側面板支持四個狀態 Tab、計數、詳情入口、篩選同步和既有任務卡操作。
- 保留七狀態領域模型、API、CLI、資料庫和實時同步鏈路。
- 全局新建任務預設進入 `todo`；欄內新建仍使用所在列狀態。
- 刪除舊的空列顯示、手動隱藏欄和“隱藏欄”執行路徑。
- 流程看板入口仍保持隱藏。

## 要求 ChatGPT Pro 修正的問題

首次交付為讓目標測試全部通過，順帶修改了評論附件和自動認領功能的舊測試斷言。這超出本次範圍。已要求並取得最小修正補丁：

- 恢復評論附件測試的基線斷言。
- 恢復自動認領測試的基線斷言。
- 只保留刪除 `BoardSettingsMenu.tsx` 所需的兩處測試改動。
- 未修改執行時代碼、依賴、鎖文件或其他測試。

## 獨立驗收

- 原始碼 ZIP 展開後密鑰掃描：`0` 條。
- ZIP 與主補丁應用後的原始碼：逐文件一致。
- 主補丁和修正補丁：`git apply --check` 通過。
- `package.json` 和 `package-lock.json`：與基線逐字節一致。
- `npm run typecheck`：通過。
- `npm run build:web`：通過。
- 當前工作樹相關合同測試：`22/22` 通過。
- 當前工作樹生產構建：通過。
- Codex 注入刷新：連接埠 `9231`，`refreshed: true`。
- 當前本機管理面板：已確認顯示三欄和“其他任務”入口。

完整 `npm test` 在安裝鎖文件完整依賴後執行：
...
- 搜索同時作用於主列和側面板。
- 面板任務遷移到主列後，刷新頁面仍保持新狀態。

未自動執行原生指針拖拽。當前瀏覽器驅動未暴露拖拽動作；已檢查真實拖拽鏈路仍重用 `TaskCard` 的 `dataTransfer` 和 `BoardColumn` 的原有 `onDrop`、排序及持久化流程。

## 當前狀態

- 修改只存在於本機 `feature/consumer-task-board` 分支工作樹。
- 未提交、未推送、未創建 PR、未部署。
