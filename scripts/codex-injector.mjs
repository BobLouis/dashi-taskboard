...
function remoteAutomationPrompt(task, comments, attachments, target) {
  ...
  return [
    ...
    attachmentText,
    "",
    "你只負責在當前遠端專案和工作目錄內完成實現與直接驗證。不要執行 taskctl，也不要訪問或修改 Taskboard。完成後返回改動、驗證結果、執行結果和剩餘限制。",
  ].join("\n");
}
    ...
    turnStarted = await requestCodexAppServerViaCdp(
      ...
      "turn/start",
      {
        ...
        input: [{
          type: "text",
          text: [
            "你是 Codex Taskboard 自動認領 Agent。只判斷下面的議題當前是否允許開始。",
            "根據完整描述和最新評論做語義判斷：若任一處明確要求等待、暫不執行或當前不應開始，decision 為 wait；否則 decision 為 start。不要呼叫工具，不要解釋。",
            JSON.stringify({
